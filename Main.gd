...
func _on_TopMenu_export_pressed():
	...
	sequencer.connect("on_note", Callable(progress, "set_value"))
...
func _on_TopMenu_save_as_pressed():
	dialog_manager.file("Save", FileDialog.FILE_MODE_SAVE_FILE, ["*.tres; Godot Text Resource FIle", "*res; Godot Resoruce File"])
	project.saved = true
	var file_path = await dialog_manager.file_dialog.file_selected 
	_save(file_path)

func _on_TopMenu_open_pressed():
	dialog_manager.file("Open", FileDialog.FILE_MODE_OPEN_FILE, ["*.tres; Godot Text Resource FIle", "*res; Godot Resoruce File"])
	var file_path = await dialog_manager.file_dialog.file_selected 
	_load(file_path)
